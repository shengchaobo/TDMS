				    		...
				    		success : function(json) {
						  		  if (typeof (json.data) != "undefined") {
										...
										if (json.checkState == WAITCHECK) {
											$("#edit").propertygrid( {
												title : '职业资质培训（<font color=red>待审核</font>）'
											});
										...
										$("#edit").propertygrid( {
											title : '职业资质培训（<font color=red>已审核通过</font>）'
										});
										$("#pass").hide();
										$("#nopass").hide();
									} else {
										$("#edit").propertygrid( {
											title : '职业资质培训（<font color=red>已审核未通过</font>）'
										});
										$("#pass").hide();
										$("#nopass").hide();
									}
	...
	$("#pass").click(function() {
		...
		//alert(checkNum);
			$.ajax( {
				type : "POST",
				url : "pages/T2103/updateCheck?selectYear=" + year + "&checkNum="+ PASSCHECK,
				async : "true",
				dataType : "text",
...
function noPassCheck(year) {
	...
	$.ajax( {
		type : "POST",
		url : "pages/T2103/updateCheck?selectYear=" + year + "&checkNum="
				+ NOPASSCHECK,
		async : "true",
		dataType : "text",
		success : function(result) {
