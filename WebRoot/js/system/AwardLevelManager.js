	...
	function newLevel() {
		...
		url = 'pages/DiAwardLevel/insert';
		$('#dlg').dialog('open').dialog('setTitle', '添加荣誉奖项级别');
	...
	function editLevel() {
		...
		url = 'pages/DiAwardLevel/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑荣誉奖项级别');
		
    	$('#IndexID').val(row[0].indexId) ;
    	$("input#IndexID").attr("readonly",true);
    	$("input#IndexID").css({"color":"#888"});
    	
    	$('#AwardLevel').val(row[0].awardLevel) ;		
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiAwardLevel/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
