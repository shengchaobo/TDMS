	...
	function newType() {
		...
		url = 'pages/DiAwardType/insert';
		$('#dlg').dialog('open').dialog('setTitle', '添加荣誉类型');
	...
	function editType() {
		...
		url = 'pages/DiAwardType/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑荣誉类型');
		
    	$('#IndexID').val(row[0].indexId) ;
    	$("input#IndexID").attr("readonly",true);
    	$("input#IndexID").css({"color":"#888"});
    	
    	$('#AwardType').val(row[0].awardType) ;		
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiAwardType/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
