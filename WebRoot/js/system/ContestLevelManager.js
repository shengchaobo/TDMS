	...
	function newLevel() {
		...
		url = 'pages/DiContestLevel/insert';
		$('#dlg').dialog('open').dialog('setTitle', '添加竞赛类型');
	...
	function validate() {
		...
		if (contestLevel == null || contestLevel.length == 0 || contestLevel == "null") {
			alert("竞赛类型名称不能为空");
			return false;
		}
	...
	function editLevel() {
		...
		url = 'pages/DiContestLevel/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑竞赛类型');
		
    	$('#IndexID').val(row[0].indexId) ;
    	$("input#IndexID").attr("readonly",true);
    	$("input#IndexID").css({"color":"#888"});
    	
    	$('#ContestLevel').val(row[0].contestLevel) ;		
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiContestLevel/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
