	...
	function newType() {
		...
		url = 'pages/DiEvaluType/insert';
		$('#dlg').dialog('open').dialog('setTitle', '添加评教类型');
	...
	function validate() {
		...
		if (evaluType == null || evaluType.length == 0 || evaluType == "null") {
			alert("评教类型不能为空");
			return false;
		}
	...
	function editType() {
		...
		url = 'pages/DiEvaluType/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑评教类型');
		
    	$('#IndexID').val(row[0].indexId) ;
    	$("input#IndexID").attr("readonly",true);
    	$("input#IndexID").css({"color":"#888"});
    	
    	$('#EvaluType').val(row[0].evaluType) ;		
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiEvaluType/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
