	...
	function newType() {
		...
		url = 'pages/DiPaperRoom/insert';
		$('#dlg').dialog('open').dialog('setTitle', '添加发表文章类别');
	...
	function validate() {
		...
		if (paperType == null || paperType.length == 0 || paperType == "null") {
			alert("发表文章类别不能为空");
			return false;
		}
	...
	function editType() {
		...
		url = 'pages/DiPaperRoom/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑发表文章类别');
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiPaperRoom/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
