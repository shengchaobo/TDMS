	...
	function editType() {
		...
		url = 'pages/DiTalentType/edit';

		$('#dlg').dialog('open').dialog('setTitle', '编辑人才类型');
		
    	$('#IndexID').val(row[0].indexId) ;
    	$("input#IndexID").attr("readonly",true);
    	$("input#IndexID").css({"color":"#888"});
    	
    	$('#TalentType').val(row[0].talentType) ;		
	...
	function deleteByIds() {
		...
		$.messager.confirm('数据删除', '您确定删除选中项?', function(sure) {
			if (sure) {
				...
				url = "pages/DiTalentType/deleteByIds?ids=" + ids ;
				submitIds();
			}
		});
	}
