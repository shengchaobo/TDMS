   ...
   var url;
	...
	function newObject() {
		...
		url = 'pages/T444/insert' 
		$('#dlg').dialog('open').dialog('setTitle', '添加新的高层次团队');
		$('#addForm').form('reset');
	}
	...
	function edit() {
	   	...
	   	$('#dlg').dialog('open').dialog('setTitle','修改高层次团队的信息');
	   	$('#seqNumber').val(row[0].seqNumber) ;
	  	$('#leader').combobox('select', row[0].leader) ;
	  	$('#resField').val(row[0].resField) ;
		$('#gainTime').datebox("setValue", formattime(row[0].gainTime)) ;	
		$('#type').combobox('setText', row[0].type) ;
		$('#otherTeamNum').val(row[0].otherTeamNum) ;	
	   	$('#otherTeamPer').val(row[0].otherTeamPer) ;
		$('#note').val(row[0].note) ;
		}
	   ...
	   function deletes(ids) {
	   	$.ajax( {
	   		type : "POST",
	   		url : "pages/T444/deleteByIds?ids=" + ids,
	   		async : "true",
	   		dataType : "text",
	   		success : function(result) {
				result = eval("(" + result + ")");
