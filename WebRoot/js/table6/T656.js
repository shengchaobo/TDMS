	...
	$(function() {
		...
		$('#newData').datagrid({
			//title : '',  //可变内容在具体页面定义
			url: 'pages/T656/loadInfo?selectYear=' + year,
			iconCls : 'icon-ok',
			width : '100%',
			//height: '100%',
		...
		//刷新页面
		 $("#cbYearContrast").combobox({  
		        onChange:function(newValue, oldValue){  
					$('#newData').datagrid( {
						//title : '',  //可变内容在具体页面定义
						url: 'pages/T656/loadInfo?selectYear=' + newValue,
						iconCls : 'icon-ok',
						width : '100%',
						//height: '100%',
						nowrap : true,//设置为true，当数据长度超出列宽时将会自动截取
		...
		//导出
		    $("#export").click(function(){
		        var tableName = encodeURI('表6-5-6学习成果-全国计算机等级考试');
		        var year = $("#cbYearContrast").combobox('getValue'); 
			    $('#exportForm').form('submit', {
			    	data : $('#exportForm').serialize(),
				    url : "pages/T656/dataExport?excelName="+tableName+'&selectYear='+year,
				    onSubmit : function() {
	...
	//单条导入
		...
		function edit() {
			...
			var year = $("#cbYearContrast").combobox('getValue'); 
		   	url = 'pages/T656/edit?selectYear='+year; 
		
			
			$('#dlg').dialog('open').dialog('setTitle', '学习成果-全国计算机等级考试');
