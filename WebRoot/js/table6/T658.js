...
function validate() {
	...
	var holdPlace = $('#holdPlace').val();
	var holdUnit = $('#holdUnit').val();
	...
	if (holdUnit == null || holdUnit.length == 0) {
		alert("举办单位不能为空");
		return false;
	}
	...
	if (holdTime == null || holdTime.length == 0) {
...
function deletes(ids) {
	$.ajax( {
		type : "POST",
		url : "pages/T658/deleteByIds?ids=" + ids,
		async : "true",
		dataType : "text",
		success : function(result) {
			result = eval("(" + result + ")");
