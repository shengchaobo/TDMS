	...
	$(function() {
		...
		$('#newData').datagrid({
			//title : '',  //可变内容在具体页面定义
			url: 'pages/T659/loadInfo?selectYear=' + year,
			iconCls : 'icon-ok',
			width : '100%',
			//height: '100%',
			...
			onLoadSuccess: function (rowData) {	
					...
					if(rowData.rows[0].checkState!=0){  								
	    				if(rowData.rows[0].checkState==WAITCHECK){
	    					if(count==0){
	    						count++;
		    					$("#newData").datagrid({title:'本科生交流情况（<font color=red>待审核</font>）'});
	    					}else{
	    					...
	    					if(count==0){
	    						count++;
		    					$("#newData").datagrid({title:'本科生交流情况（<font color=red>审核通过</font>）'});
	    					}else{
	    					...
	    					if(count==0){
	    						count++;
		    					$("#newData").datagrid({title:'本科生交流情况（<font color=red>审核未通过</font>）'});
	    					}else{
    					...
    					if(count==0){
    						count++;
        					$("#newData").datagrid({title:'本科生交流情况'});
    					}else{
	...
	//刷新页面
	 $("#cbYearContrast").combobox({  
	        onChange:function(newValue, oldValue){  
				$('#newData').datagrid( {
					//title : '',  //可变内容在具体页面定义
					url: 'pages/T659/loadInfo?selectYear=' + newValue,
					iconCls : 'icon-ok',
					width : '100%',
					//height: '100%',
					nowrap : true,//设置为true，当数据长度超出列宽时将会自动截取
	   ...
	   //导出
	    $("#export").click(function(){
	        var tableName = encodeURI('表6-5-9本科生交流情况（教学单位-国际交流与合作处）');
	        var year = $("#cbYearContrast").combobox('getValue'); 
		    $('#exportForm').form('submit', {
		    	data : $('#exportForm').serialize(),
			    url : "pages/T659/dataExport?excelName="+tableName+'&selectYear='+year,
			    onSubmit : function() {
	...
	function edit() {
	   	...
	   	var year = $("#cbYearContrast").combobox('getValue'); 
	   	url = 'pages/T659/edit?selectYear='+year; 
    	...
    	$('#exchangeStuSum').numberbox('setValue',row[0].exchangeStuSum); 
	  	$('#fromSchToOverseas').numberbox('setValue',row[0].fromSchToOverseas); 
	  	$('#fromSchToDomestic').numberbox('setValue',row[0].fromSchToDomestic); 
	  	$('#fromDomesticToSch').numberbox('setValue',row[0].fromDomesticToSch); 
	  	$('#fromOverseasToSch').numberbox('setValue',row[0].fromOverseasToSch); 
