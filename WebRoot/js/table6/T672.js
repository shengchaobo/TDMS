...
//弹出添加的界面
function newItem() {
	url = 'pages/T672/insert' ; 
	$('.title1').show();
	$('#item1').show();
	$('hr').show();
	$('#dlg').dialog('open').dialog('setTitle', '双学位情况汇总');
...
function validate() {
	...
	var dualDegreeUnitId = $('#dualDegreeUnitId').combobox('getText');
	var dualDegreeId = $('#dualDegreeId').combobox('getText');
	...
	if (unitId == null || unitId.length == 0 || dualDegreeUnitId == null || dualDegreeUnitId.length == 0) {
		alert("所在教学单位不能为空");
		return false;
	}
	
	if (majId == null || majId.length == 0 || dualDegreeId == null || dualDegreeId.length == 0) {
		alert("所在专业或双学位专业不能为空");
		return false;
	}
...
function deletes(ids) {
	$.ajax( {
		type : "POST",
		url : "pages/T672/deleteByIds?ids=" + ids,
		async : "true",
		dataType : "text",
		success : function(result) {
			result = eval("(" + result + ")");
