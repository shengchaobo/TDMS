...
<meta http-equiv="description" content="This is my page">
  ...
  function passCheck(seqNumber){
		    ...
		    $.ajax({
				    type:"POST", 
				    url: "pages/MainTrainBasicInfoTea/updateCheck?seqNum=" + seqNumber +"&checkNum=" + checkNum, 
			   		async : "true",
			   		dataType : "text",
  ...
  function checkAll(){
		    ...
		    $.ajax({
				    type:"POST", 
				    url: "pages/MainTrainBasicInfoTea/checkAll", 
			   		async : "true",
			   		dataType : "text",
				    success:function(result){  
				         result = eval("(" + result + ")");						    	 
...
<body style="height: 100%'">
  <% request.setAttribute("WAITCHECK",Constants.WAIT_CHECK); %>
	<table  id="checkData"  class="easyui-datagrid"  url="pages/MainTrainBasicInfoTea/auditingData?checkNum=<%=request.getAttribute("WAITCHECK")%>"   style="height: auto"  >
		...
		<table  id="checkPassData"  class="easyui-datagrid"  url="pages/MainTrainBasicInfoTea/auditingData?checkNum=<%=request.getAttribute("PASSCHECK")%>&checkFlag=1"   style="height: auto"  >
		<thead data-options="frozen:true">
			<tr>			
			...
			<table>
				...
				<tr>
					<td>
						<div class="fitem">
							<label>被审核表ID：</label> 
								<input type="text" name="checkInfo.tableID" id="tableName"   value="T321"
								readonly="readonly"  style="width: 150px;color: grey"/>
							</div>
					</td>
				</tr>
