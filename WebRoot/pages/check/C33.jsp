...
<meta http-equiv="description" content="This is my page">
  ...
  function passCheck(seqNumber){
		    ...
		    $.ajax({
				    type:"POST", 
				    url: "pages/JuniorMajInfo/updateCheck?seqNum=" + seqNumber +"&checkNum=" + checkNum, 
			   		async : "true",
			   		dataType : "text",
  ...
  function checkAll(){
  		  if(confirm("全部数据审核通过，该操作不可恢复，确认吗？")){ 		  	
			    $.ajax({
					    type:"POST", 
					    url: "pages/JuniorMajInfo/checkAll", 
				   		async : "true",
				   		dataType : "text",
					    success:function(result){  
					         result = eval("(" + result + ")");						    	 
...
<body style="height: 100%'">
	<table  id="checkData"  class="easyui-datagrid"  url="pages/JuniorMajInfo/auditingData?checkNum=<%=request.getAttribute("WAITCHECK")%>"   style="height: auto"  >
	...
	<table  id="checkPassData"  class="easyui-datagrid"  url="pages/JuniorMajInfo/auditingData?checkNum=<%=request.getAttribute("PASSCHECK")%>&checkFlag=0"   style="height: auto"  >
		<thead data-options="frozen:true">
			<tr>			
			...
			<table>			
				...
				<tr>
					<td>
						<div class="fitem">
							<label>被审核表ID：</label> 
								<input type="text" name="checkInfo.tableID" id="tableName"   value="T33"
								readonly="readonly"  style="width: 150px;color: grey"/>
							</div>
					</td>
				</tr>
