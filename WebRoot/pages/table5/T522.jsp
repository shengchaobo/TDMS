...
<body style="height: 100%'" onload="myMarquee('T522','<%=request.getAttribute("CHECKTYPE") %>')">
<div  id="floatDiv">
  ...
  <br/> 
	...
	<table  id="unverfiedData"  class="easyui-datagrid"  url="pages/T522/auditingData?checkNum=<%=request.getAttribute("NOCHECK") %>"   style="height: auto"  >
	...
	<table id="verfiedData"  class="easyui-datagrid"  url="pages/T522/auditingData?checkNum=<%=request.getAttribute("PASS") %>"  style="height: auto;" >
		<thead data-options="frozen:true">
			<tr>	
			<th data-options="field:'ck',checkbox:true">选取</th>		
				<th  data-options="field:'seqNumber'" >编号</th>
	...
	<form action='pages/T522/dataExport?excelName=<%=URLEncoder.encode("表5-2-2网络课程建设情况（网络信息中心）","UTF-8")%>'   method="post"  id="exportForm" enctype="multipart/form-data"  style="float: right;">
		...
		<table>
			<tr>
				<td>
					<input type="hidden" name="t522Bean.SeqNumber"  id="seqNumber" value="0"/>
						<input type="hidden" name="t522Bean.Time"  id="Time" value="0"/>
					<div class="fitem">
						<label>类型：</label> 
			...
			<tr>
				<td>
					<div class="fitem">
					<label>负责人教工号：</label> 
					<input type="hidden" name="t522Bean.TeaID" id="TeaID"/>
						<input id="Leader" type="text" name="t522Bean.Leader" class='easyui-combobox' 
