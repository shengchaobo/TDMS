 ...
 <div  id="floatDiv">
  ...
  <br/>
	...
	<div id="dlg" class="easyui-dialog"
		...
		buttons="#dlg-buttons">
		<h3 class="title1">院（系）教学工作状态评估批量导入</h3>
		<div class="fitem" id="item1">
			<form method="post" id="batchForm" enctype="multipart/form-data">
			<select class="easyui-combobox"  id="cbYearContrast" name="selectYear"></select>
				<input type="file" name="uploadFile" id="fileToUpload" class="easyui-validatebox" size="48" style="height: 24px;" required="true" />
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" onclick="batchImport()">模板导入</a>
				<a href="pages/T745/downloadModel?saveFile=<%=URLEncoder.encode("表7-4-5院（系）教学工作状态评估.xls","UTF-8")%>"  class="easyui-linkbutton" iconCls="icon-download">模板下载</a>
			</form>
			<a href="123"></a>
		</div>
	   ...
	   <hr></hr>	
		...
		<h3 class="title1">院（系）教学工作状态评估逐条导入</h3>
		...
		<table>
			...
			<tr>
				<td>
					<div class="fitem">
						<label>评估结果：</label> 
						<select class='easyui-combobox' id="AssessResult" name="teachWorkAssessAC.AssessResult" panelHeight="auto" editable="false">
	 ...
	 function batchImport(){
	    ...
	    function newCourse(){
	     $('.title1').show();
	    	$('#item1').show();
	    	$('hr').show();
	       url="pages/T745/insert";
		    $('#dlg').dialog('open').dialog('setTitle','添加院（系）教学工作状态评估情况');
       ...
       function deleteCourses(ids){
	    	$.ajax({ 
	    		type: "POST", 
	    		url: "pages/T745/deleteByIds?ids=" + ids, 
	    		async:"true",
	    		dataType: "text",
	    		success: function(result){
	    			result = eval("(" + result + ")");
