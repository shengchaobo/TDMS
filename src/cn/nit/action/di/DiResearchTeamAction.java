...
public class DiResearchTeamAction {
	...
	private DiResearchTeamBean team_bean = new DiResearchTeamBean();
	private String ids ;
	//查出所有
	...
	public void edit(){
		...
		boolean flag = ResearchTeamServices.update(team_bean) ;
	...
	public void insert(){
		...
		//首先该用 户是否已存数据库
		Boolean flag0 = ResearchTeamServices.hasTeam(team_bean.getIndexId());
		
		boolean flag = false;
		//如果该用户不存数据库，可以添加
		if(flag0 == false){			
			flag = ResearchTeamServices.insert(team_bean) ;
		}
		...
		try{
			...
			if(flag0){
				out.print("{\"state\":true,data:\"该研究团队类型已存在!!!\"}") ;
			}else{
				if(flag){
	...
	public void deleteByIds(){
		...
		boolean flag = ResearchTeamServices.deleteByIds(ids) ;
		PrintWriter out = null ;
	...
	public void setTeam_bean(DiResearchTeamBean team_bean) {
		this.team_bean = team_bean;
	}

	public DiResearchTeamBean getTeam_bean() {
		return team_bean;
	}
