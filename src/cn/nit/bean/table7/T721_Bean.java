package cn.nit.bean.table7;

import java.util.Date;

public class T721_Bean {
	
	private int SeqNumber ;
	
	private String ItemName;
	
	private String TeaUnit;
	
	private String UnitID;
	
	private String Leader;
	
	private String TeaID;
	
	private int OtherTeaNum;
	
	private String OtherTea;
	
	private String ItemLevel;
	
	private Date ItemSetUpTime;
	
	private Date ReceptTime;
	
	private double ApplvExp;
	
	private double SchSupportExp;
	
	private String AppvlID;
	
	private Date Time;
	
	private String Note;

	
	
	public int getSeqNumber() {
		return SeqNumber;
	}

	public void setSeqNumber(int seqNumber) {
		SeqNumber = seqNumber;
	}

	public String getItemName() {
		return ItemName;
	}

	public void setItemName(String itemName) {
		ItemName = itemName;
	}

	public String getTeaUnit() {
		return TeaUnit;
	}

	public void setTeaUnit(String teaUnit) {
		TeaUnit = teaUnit;
	}

	public String getUnitID() {
		return UnitID;
	}

	public void setUnitID(String unitID) {
		UnitID = unitID;
	}

	public String getLeader() {
		return Leader;
	}

	public void setLeader(String leader) {
		Leader = leader;
	}

	public String getTeaID() {
		return TeaID;
	}

	public void setTeaID(String teaID) {
		TeaID = teaID;
	}

	public int getOtherTeaNum() {
		return OtherTeaNum;
	}

	public void setOtherTeaNum(int otherJoinTeaNum) {
		OtherTeaNum = otherJoinTeaNum;
	}

	public String getOtherTea() {
		return OtherTea;
	}

	public void setOtherTea(String otherTea) {
		OtherTea = otherTea;
	}

	public String getItemLevel() {
		return ItemLevel;
	}

	public void setItemLevel(String itemLevel) {
		ItemLevel = itemLevel;
	}

	public Date getItemSetUpTime() {
		return ItemSetUpTime;
	}

	public void setItemSetUpTime(Date itemSetUpTime) {
		ItemSetUpTime = itemSetUpTime;
	}

	public Date getReceptTime() {
		return ReceptTime;
	}

	public void setReceptTime(Date receptTime) {
		ReceptTime = receptTime;
	}

	public double getApplvExp() {
		return ApplvExp;
	}

	public void setApplvExp(double applvExp) {
		ApplvExp = applvExp;
	}

	public double getSchSupportExp() {
		return SchSupportExp;
	}

	public void setSchSupportExp(double schSupportExp) {
		SchSupportExp = schSupportExp;
	}

	public String getAppvlID() {
		return AppvlID;
	}

	public void setAppvlID(String appvlID) {
		AppvlID = appvlID;
	}

	public Date getTime() {
		return Time;
	}

	public void setTime(Date time) {
		Time = time;
	}

	public String getNote() {
		return Note;
	}

	public void setNote(String note) {
		Note = note;
	}
	
	
	
	
	

}
