...
public class DiTitleLevelDao {

	private String tableName = "DiTitleLevel" ;
	
	private String field = "IndexID,TitleLevel" ;
	...
	public List<DiTitleLevelBean> getList(){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<DiTitleLevelBean> list = null ;
		
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, DiTitleLevelBean.class) ;
		}catch(Exception e){
			e.printStackTrace() ;
			return null ;
		}finally{
	 ...
	 */
	public boolean insert(DiTitleLevelBean titleLevel){
		
		Connection conn = DBConnection.instance.getConnection() ;
		return DAOUtil.insert(titleLevel, tableName, field, conn) ;
	}
