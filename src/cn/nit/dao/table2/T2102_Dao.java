...
public class T2102_Dao {
	
	private String tableName = "T2102_JobEduCouser_Admission$" ;
	private String field = "CourseNum,CourseTimes,TeaNum,StuNum,Time,Note,CheckState";
	 ...
	 */
	public T2102_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T2102_Bean> list = null ;
		T2102_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T2102_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
