...
public class T2103_Dao {
	
	private String tableName = "T2103_ProfTrain_Prof$" ;
	private String field = "StuProfTrainNum,Time,Note,CheckState";
	 ...
	 */
	public T2103_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T2103_Bean> list = null ;
		T2103_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T2103_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
