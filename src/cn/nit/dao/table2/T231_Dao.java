...
public class T231_Dao {
	
	private String tableName = "T231_Classroom_Tea$" ;
	private String field = "ClassrmArea,ClassNum,ComputerNum,MediaNum,ClassSeatNum,ComputerSitNum,MediaSitNum,Time,Note,CheckState";
	 ...
	 */
	public T231_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T231_Bean> list = null ;
		T231_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T231_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
