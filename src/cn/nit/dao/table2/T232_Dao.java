...
public class T232_Dao {
	
	private String tableName = "T232_ComputerNum_EQU$" ;
	private String field = "ComputerRoom,ComputerNum,Time,Note,CheckState";
	 ...
	 */
	public T232_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T232_Bean> list = null ;
		T232_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T232_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
