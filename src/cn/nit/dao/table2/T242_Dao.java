...
public class T242_Dao {
	
	private String tableName = "T242_CurYearAddBook_Lib$" ;
	private String field = "AddPaperBookNum,AddDigBookType,LiterAcqusExps,BookTurnover,DigResVisit,Time,Note,CheckState";
	 ...
	 */
	public T242_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T242_Bean> list = null ;
		T242_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T242_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
