...
public class T291_Dao {
	
	private String tableName = "T291_TeaExpInfo_Finance$" ;
	private String field = "SumTeaExp,TeaExpBudget,SpecialExp,Time,Note,CheckState";
	 ...
	 */
	public T291_Bean getYearInfo(String year){
		...
		Statement st = null ;
		ResultSet rs = null ;
		List<T291_Bean> list = null ;
		T291_Bean bean = null;
		try{
			st = conn.createStatement() ;
			rs = st.executeQuery(sql) ;
			list = DAOUtil.getList(rs, T291_Bean.class) ;
			if(list.size() != 0){
				bean = list.get(0);
			}
		}catch(Exception e){
