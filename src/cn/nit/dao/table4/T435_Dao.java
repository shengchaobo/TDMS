...
public class T435_Dao {
	
	private String tableName = "T435_EmployManager_Admission$" ;
	private String field = "Name,TeaId,FromDept,UnitId,StaffType,Time,Note,CheckState";
	 ...
	 */
	public List<T435_Bean> totalList(String year, int checkState){
		...
		try{
			...
			list = DAOUtil.getList(rs, T435_Bean.class) ;
		}catch(Exception e){
			e.printStackTrace() ;
			return null ;
		}finally{
	 ...
	 * 
	 */
	public List<T435_Bean> queryPageList(String conditions, String fillunitID, int pageSize, int showPage){
	 ...
	 */
	public boolean insert(T435_Bean schLeader){
	...
	public boolean update(T435_Bean bean){
		...
		Connection conn = DBConnection.instance.getConnection() ;
		try{
			String tempfield = "Name,TeaId,FromDept,UnitId,StaffType,Note,CheckState";
			
			flag = DAOUtil.update(bean, tableName, keyfield, tempfield, conn) ;
		}catch(Exception e){
			e.printStackTrace() ;
			return flag ;
		}finally{
